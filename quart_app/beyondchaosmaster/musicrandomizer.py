...
RUN_PATH = "quart_app/beyondchaosmaster"
...
def process_custom_music(data_in, eventmodes="", opera=None, f_randomize=True, f_battleprog=True, f_mchaos=False, f_altsonglist=False):
    ...
    while keeptrying and attempts <= 1000:
        ...
        for ident, s in songtable.items():
            ...
            # CHANGED TO OS.PATH.EXISTS TO CHECK
            elif os.path.exists(os.path.join(MUSIC_PATH, s.changeto + ".mml")) or os.path.exists(os.path.join(MUSIC_PATH, usage_id(s.changeto) + ".mml")) or os.path.exists(os.path.join(MUSIC_PATH, vbase + vid + ".mml")):
